...
func safeGetTracks(
	...
	if q := strings.TrimSpace(m.Args()); isPlaylistQuery(q) {
		tracks, err := playlistTracks(q)
		if err != nil {
			return nil, errors.New(F(chatID, "playlist_not_found"))
		}
		if len(tracks) == 0 {
			return nil, errors.New(F(chatID, "playlist_songs_empty"))
		}
		return tracks, nil
	}
...
func autoplayHandler(c *td.Client, m *td.Message) error {
	...
	chatID := m.ChatID()
	var enabled bool
	if r, ok := core.RoomFor(chatID); ok && r.Active() {
		enabled = r.Autoplay()
	} else {
		enabled, _ = database.Autoplay(chatID)
	}
...
func autoplayCallbackHandler(c *td.Client, u *td.UpdateNewCallbackQuery) error {
	...
	var enabled bool
	r, hasRoom := core.RoomFor(chatID)
	if hasRoom && r.Active() {
		enabled = !r.Autoplay()
		r.SetAutoplay(enabled)
	} else {
		curr, _ := database.Autoplay(chatID)
		enabled = !curr
