...
func init() {
	helpTexts["/repo"] = `<i>Show bot source code repository and developer info.</i>`
}

func repoHandler(c *td.Client, m *td.Message) error {
	if isChannel(c, m) {
		return nil
	}
	...
	if config.DevURL != "" {
		...
		var ownerUser *td.User
		if user, err := c.GetUser(config.OwnerID); err == nil && user != nil {
			ownerUser = user
		} else {
			if _, err := c.CreatePrivateChat(config.OwnerID, nil); err == nil {
				if user, err := c.GetUser(config.OwnerID); err == nil && user != nil {
					ownerUser = user
				}
			}
		...
		if ownerUser != nil && ownerUser.Usernames != nil {
			if len(ownerUser.Usernames.ActiveUsernames) > 0 && ownerUser.Usernames.ActiveUsernames[0] != "" {
				devURL = "https://t.me/" + ownerUser.Usernames.ActiveUsernames[0]
			} else if ownerUser.Usernames.EditableUsername != "" {
				devURL = "https://t.me/" + ownerUser.Usernames.EditableUsername
			}
		}
	...
	botName := "Kanha Music"
	if c.Me != nil && c.Me.FirstName != "" {
		botName = c.Me.FirstName
	}

	caption := F(chatID, "repo_text", locales.Arg{
		"bot": botName,
	})

	markup := core.GetRepoMarkup(chatID, devURL)

	img := config.RepoImage
	if img == "" {
		img = config.StartImage()
	}
	if img == "" {
		img = config.PingImage
	}

	if img != "" {
		lower := strings.ToLower(img)
		if strings.HasSuffix(lower, ".mp4") || strings.HasSuffix(lower, ".mkv") || strings.HasSuffix(lower, ".webm") {
			if _, err := m.ReplyVideo(c, td.InputFileRemote{Id: img}, &td.SendVideoOpts{
				Caption:           caption,
				ParseMode:         td.ParseModeHTML,
				ReplyMarkup:       markup,
				...
				logger.Warnf("[repo] ReplyVideo failed: %v", err)
			}
		}

		if _, err := m.ReplyPhoto(c, td.InputFileRemote{Id: img}, &td.SendPhotoOpts{
			Caption:     caption,
			ParseMode:   td.ParseModeHTML,
			ReplyMarkup: markup,
		}); err == nil {
			...
			logger.Errorf("[repo] ReplyPhoto failed: %v", err)
		}
	}
...
// ©️ Copyright Reserved - KanhaMusic
// ===========================================
// ©️ 2026 Kanha (aka @Oyekanhaa)
// 🔗 GitHub : https://github.com/Oyekanhaa/KanhaMusic
// 📢 Telegram Channel : https://t.me/MeowCloner
// 💬 Support Group : https://t.me/MeowClone
// ===========================================
// ❤️ Love From KanhaMusic
